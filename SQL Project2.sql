...
--4.  Display product criteria
	--C1 : quantity packages in bottles
	select PRODUCT_REF, 
			PRODUCT_NAME AS 'C1', 
	...
	where PRODUCT_NAME LIKE upper('__t%')
	...
	select PRODUCT_NAME as 'C4' from PRODUCTS WHERE UNIT_PRICE BETWEEN 70 AND 200;
...
WHERE 
    ...
    AND PRODUCT_NAME LIKE UPPER('__t%')
...
select CUSTOMER_CODE,  ADDRESS, CITY, POSTAL_CODE, COUNTRY FROM CUSTOMERS
WHERE EXISTS (
select 1 
FROM SUPPLIERS 
WHERE  CUSTOMERS.ADDRESS = SUPPLIERS.ADDRESS
AND CUSTOMERS.CITY = SUPPLIERS.CITY
AND CUSTOMERS.POSTAL_CODE = SUPPLIERS.POSTAL_CODE
...
select ORDER_NUMBER, UNIT_PRICE*QUANTITY as 'full price', DISCOUNT*100 AS 'New_Discount_Rate %', DISCOUNT*100 AS 'DISCOUNT RATE' FROM ORDER_DETAILS WHERE UNIT_PRICE*QUANTITY <= 10
UNION
select ORDER_NUMBER, UNIT_PRICE*QUANTITY as 'full price', DISCOUNT*100+5 AS 'New_Discount_Rate %', DISCOUNT*100 AS 'DISCOUNT RATE' FROM ORDER_DETAILS WHERE UNIT_PRICE*QUANTITY BETWEEN 11 AND 20
UNION
select ORDER_NUMBER, UNIT_PRICE*QUANTITY as 'full price', DISCOUNT*100+10 AS 'New_Discount_Rate %', DISCOUNT*100 AS 'DISCOUNT RATE' FROM ORDER_DETAILS WHERE UNIT_PRICE*QUANTITY BETWEEN 21 AND 30
UNION
select ORDER_NUMBER, UNIT_PRICE*QUANTITY as 'full price', DISCOUNT*100+15 AS 'New_Discount_Rate %', DISCOUNT*100 AS 'DISCOUNT RATE' FROM ORDER_DETAILS WHERE UNIT_PRICE*QUANTITY >=31 ORDER BY ORDER_NUMBER ASC 
...
select SUPPLIERS.SUPPLIER_NUMBER, COMPANY, ADDRESS, SUPPLIERS.PHONE FROM SUPPLIERS 
INNER JOIN PRODUCTS
ON PRODUCTS.SUPPLIER_NUMBER = SUPPLIERS.SUPPLIER_NUMBER

INNER JOIN CATEGORIES
ON CATEGORIES.CATEGORY_CODE = PRODUCTS.CATEGORY_CODE 
WHERE CATEGORIES.CATEGORY_CODE = 'Beverages';


SELECT* FROM SUPPLIERS
SELECT* FROM PRODUCTS
SELECT* FROM CATEGORIES
SELECT CATEGORY_CODE+100 'category code', CATEGORY_NAME, DESCRIPTION FROM CATEGORIES
SELECT description FROM categories where DESCRIPTION = 'desserts and sweet treats'
...
SELECT CUSTOMER_CODE FROM CUSTOMERS WHERE CITY = 'Seattle'
